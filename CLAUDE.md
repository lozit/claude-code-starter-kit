...
- Every template carries `<!-- generated-by: groundrules v1.10.0 -->` at the top.
...
- **Also at release time, run the eval suite** if this release touched the agent's configuration (`skills/**`, their templates, this file): `claude plugin eval .` ([ADR 0037](docs/decisions/0037-executable-evals-over-agent-config.md)). Same trigger on a PR that changes those paths. It is **out of band** — a command you run, never a hook (ADR 0025 stands). A red case is a signal to read, not yet a release blocker.
