...
2. **Detect superpowers**: presence of `docs/superpowers/plans/` or `specs/`. If so → different altitude, **do not** treat as root planning (see interop in the `CLAUDE.md` template). Set `HAS_SUPERPOWERS` from it, falling back to the installed-plugin registry exactly as `bootstrap` Phase 1 — it decides whether a generated `CLAUDE.md` carries the interop section, and it is recorded as `answers.usesSuperpowers`. On a brownfield project signal 1 is usually decisive, which is why `adopt` rarely needs the fallback.
